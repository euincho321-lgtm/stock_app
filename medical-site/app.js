...
function buildFallback(data) {
  return `주증상에 대한 자동 분류 데이터가 아직 충분하지 않습니다.

입력된 주증상: ${data.chiefComplaint}

1. 감별진단을 위해 추가적으로 물어봐야 할 질문
...
4. 가장 의심되는 진단에 대한 정리
- 현재 정보만으로는 특정 진단을 1순위로 확정하기 어려움
- 추가 문진과 진찰 후 대증치료 여부를 판단
- Red flag가 있으면 즉시 상급 평가 필요

주의: 이 출력은 일반 프레임이며, 개별 환자 진료 판단을 대체하지 않습니다.`;
}
...
1. 감별진단을 위해 추가적으로 물어봐야 할 질문
${toBulletList(matched.questions)}

2. 의심되는 진단
${toBulletList(matched.diagnoses)}

3. 정확한 진단을 위해 추가적으로 필요한 검사
${toBulletList(matched.tests)}

4. 가장 의심되는 진단에 대한 정리
- 진단명: ${matched.leading.diagnosis}
- 이유: ${matched.leading.rationale}
- 약물처방:
${toBulletList(matched.leading.medication)}
- 생활요법:
${toBulletList(matched.leading.lifestyle)}
- 주의사항:
${toBulletList(matched.leading.cautions)}

면책: 이 결과는 외래 진료 보조용 참고 자료이며 최종 진단과 처방은 의료진의 직접 판단이 필요합니다.`;
}
